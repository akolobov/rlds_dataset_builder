...
from bridge_data_MSR.conversion_utils import MultiThreadedDatasetBuilder


N_VIEWS = 3
IMAGE_SIZE = (480, 640)
INPUT_PATH = "/mnt/d/bridge_data_v2__MSR/"
DEPTH = 2
...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    def _parse_examples(episode_path, camera_topics):
        ...
        def process_lang(path):
            ...
            task = path[-5]
            suffix_idx = task.find("_VARIOUS")
            if suffix_idx > -1:
                task = task[:suffix_idx]
            suffix_idx = task.find("_(")
            if suffix_idx > -1:
                task = task[:suffix_idx]
            if task == "PLAY_DATA":
                text = ""
            elif task == "pick_and_place":
                text = "put the object on the plate"
            else:
                text = ' '.join(task.split('_'))
            ...
            episode.append(
                {
                    ...
                    "discount": 1.0,
                    "reward": (float(i == (len(out["actions"]) - 1)) if instruction else 0.0),
                    "is_first": i == 0,
                    "is_last": i == (len(out["actions"]) - 1),
                    "is_terminal": (i == (len(out["actions"]) - 1) if instruction else False),
...
class BridgeDataMSR(MultiThreadedDatasetBuilder):
    ...
    N_WORKERS = 1 #4 #2 #10  # number of parallel workers for data conversion
    MAX_PATHS_IN_MEMORY = (
        10 #1  # number of paths converted & stored in memory before writing to disk
    )
    ...
    def _split_paths(self):
        ...
        # each path is basically a *task* path
        for path in paths:
            for dated_folder in os.listdir(path):
                ...
                if not all_traj:
                    print(f"No episodes found in {search_path}")
                    continue
